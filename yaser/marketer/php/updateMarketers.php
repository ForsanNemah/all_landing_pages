<?php
	...
	$state= $_REQUEST['state']; //userId ID we are using it to get the user record

	include("../../include/connection.php");

	// Set the INSERT SQL data
	$sql = "UPDATE `marketers` SET `name`='$name',`password`='$password',`block`='$state' WHERE id='".$id."'";
       
	// Process the query so that we will update data
	if ($mysqli->query($sql)) {
	  echo "Markters has been sucessfully updated.";
	} else {
